...
class LateScreenshotUploadTestCase(TestCase):
    ...
    def create_large_image(self, size_mb=6):
        ...
        # A solid-color image compresses to just a few KB under JPEG no matter
        # how many pixels it has, so build a small valid JPEG and pad it with
        # trailing bytes (ignored by decoders/validators past the EOI marker)
        # to actually reach the target size on disk.
        file = BytesIO()
        ...
        if len(content) < target_size:
            content += b'\x00' * (target_size - len(content))
        return SimpleUploadedFile(
            'large_test.jpg',
            content,
