...
export default function HelpScreen() {
  ...
  const renderFAQItem = (faq: FAQ) => (
    <TouchableOpacity
      key={faq.id}
      style={[styles.faqItem, { backgroundColor: theme.card, borderBottomColor: theme.border }]}
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {contactInfo && (
            <View style={styles.contactButtons}>
              <TouchableOpacity
                style={[styles.contactButton, { backgroundColor: theme.card, borderColor: theme.border }]}
          ...
          <View style={[styles.section, styles.contactSection, { backgroundColor: theme.card }]}>
            ...
            <View style={styles.contactInfo}>
              <View style={styles.contactRow}>
                ...
                <Text style={[styles.contactText, { color: theme.textSecondary }]}>{contactInfo.email}</Text>
              </View>
              <View style={styles.contactRow}>
                <Ionicons name="call" size={16} color={theme.textSecondary} />
                <Text style={[styles.contactText, { color: theme.textSecondary }]}>{contactInfo.phone}</Text>
              </View>
              <View style={styles.contactRow}>
                <Ionicons name="time" size={16} color={theme.textSecondary} />
                <Text style={[styles.contactText, { color: theme.textSecondary }]}>{contactInfo.business_hours}</Text>
              </View>
              <View style={styles.contactRow}>
                <Ionicons name="shield-checkmark" size={16} color={theme.textSecondary} />
                <Text style={[styles.contactText, { color: theme.textSecondary }]}>Emergency: {contactInfo.support_hours}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryScroll} contentContainerStyle={styles.categoryScrollContent}>
            {categories.map((category) => (
              <TouchableOpacity
                key={category.value}
                style={[styles.categoryButton, { backgroundColor: selectedCategory === category.value ? theme.primary : theme.card, borderColor: theme.border }]}
                onPress={() => setSelectedCategory(category.value)}
              >
                <Ionicons name={category.icon} size={20} color={selectedCategory === category.value ? 'white' : theme.textSecondary} />
                <Text style={[styles.categoryButtonText, { color: selectedCategory === category.value ? 'white' : theme.text }]}>
                  {category.label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push('/(worker)/terms' as any)}
          >
          ...
          <TouchableOpacity
            style={[styles.resourceButton, { backgroundColor: theme.card, borderColor: theme.border }]}
            onPress={() => router.push('/(worker)/privacy' as any)}
          >
...
const styles = StyleSheet.create({
  ...
  section: { padding: 16 },
  contactSection: { marginHorizontal: 16, marginBottom: 16, borderRadius: 12, padding: 16 },
  sectionTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 16 },
  actionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 12, marginBottom: 16, gap: 8 },
  actionButtonText: { color: 'white', fontSize: 16, fontWeight: '600' },
  contactButtons: { flexDirection: 'row', gap: 12 },
  contactButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 12, borderRadius: 8, borderWidth: 1, gap: 8 },
  contactButtonText: { fontSize: 14, fontWeight: '500' },
  contactInfo: { gap: 12 },
  contactRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  contactText: { fontSize: 14 },
  categoryScroll: { marginBottom: 16 },
  categoryScrollContent: { paddingHorizontal: 4, gap: 8 },
  categoryButton: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, borderWidth: 1, gap: 6 },
  categoryButtonText: { fontSize: 14, fontWeight: '500' },
  faqContainer: { borderRadius: 12, overflow: 'hidden' },
  faqItem: { padding: 16, borderBottomWidth: 1 },
  faqHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  faqQuestion: { flex: 1, fontSize: 16, fontWeight: '500', marginRight: 8 },
  faqAnswer: { marginTop: 12, fontSize: 14, lineHeight: 20 },
  emptyContainer: { padding: 48 },
  emptyTitle: { fontSize: 18, fontWeight: 'bold', marginTop: 16, textAlign: 'center' },
  emptySubtitle: { fontSize: 14, marginTop: 8, textAlign: 'center' },
  resourceButton: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 12, borderWidth: 1, marginBottom: 12, gap: 12 },
  resourceText: { flex: 1 },
  resourceTitle: { fontSize: 16, fontWeight: '600', marginBottom: 4 },
  resourceSubtitle: { fontSize: 14 },
});
