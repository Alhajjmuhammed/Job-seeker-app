...
export default function SettingsScreen() {
  ...
  const loadSettings = async () => {
    try {
      ...
      // Backend is the source of truth for the fields it actually tracks -
      // reconcile in case this is a new device or settings changed elsewhere.
      try {
        const backend = await apiService.getPrivacySettings();
        setNotificationSettings((prev) => ({
          ...prev,
          pushEnabled: backend.push_notifications ?? prev.pushEnabled,
          emailEnabled: backend.email_notifications ?? prev.emailEnabled,
          smsEnabled: backend.sms_notifications ?? prev.smsEnabled,
          marketingEmails: backend.marketing_emails ?? prev.marketingEmails,
        }));
        setPrivacySettings((prev) => ({
          ...prev,
          profileVisible: backend.profile_visibility === 'public',
          showPhoneNumber: backend.show_phone ?? prev.showPhoneNumber,
        }));
      } catch (backendError) {
        console.error('Error loading backend privacy settings:', backendError);
      }
    } catch (error) {
  ...
  const saveNotificationSettings = async (newSettings: NotificationSettings) => {
    try {
      ...
      // Push/email/sms/marketing map directly to real backend preference
      // fields - persist them server-side, not just on this device.
      // (jobAlerts/messageAlerts/applicationUpdates have no backend
      // equivalent yet - more granular than what the API supports.)
      await apiService.updatePrivacySettings({
        push_notifications: newSettings.pushEnabled,
        email_notifications: newSettings.emailEnabled,
        sms_notifications: newSettings.smsEnabled,
  ...
  const savePrivacySettings = async (newSettings: PrivacySettings) => {
    try {
      ...
      // profileVisible/showPhoneNumber map to real backend fields.
      // (showLocation/allowDirectMessages have no backend equivalent yet.)
      await apiService.updatePrivacySettings({
        profile_visibility: newSettings.profileVisible ? 'public' : 'private',
        show_phone: newSettings.showPhoneNumber,
      });
    } catch (error) {
