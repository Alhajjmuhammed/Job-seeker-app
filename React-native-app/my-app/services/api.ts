...
class ApiService {
  ...
  async acceptDirectHireRequest(requestId: number) {
    // Registered under jobs.api_urls, not workers.api_urls like the list endpoint above
    const response = await this.api.post(`/jobs/worker/direct-hire-requests/${requestId}/accept/`);
    return response.data;
  }

  async rejectDirectHireRequest(requestId: number) {
    const response = await this.api.post(`/jobs/worker/direct-hire-requests/${requestId}/reject/`);
    return response.data;
  }
  ...
  async getBrowseJobs(params?: { category?: string; city?: string }) {
    // jobs.api_urls registers this as 'jobs/browse/' (its own extra prefix,
    // on top of the 'api/jobs/' mount) - '/jobs/browse/' alone 404s.
    const response = await this.api.get('/jobs/jobs/browse/', { params });
    return response.data;
  }

  // For workers viewing a job from Browse Jobs - client_job_detail (used by
  // getClientJobDetail/getJobDetail below) is scoped to the owning client
  // and always 404s for a worker. This hits the permission-checked
  // endpoint that allows the owner, an assigned worker, or staff.
  async getWorkerJobDetail(jobId: number) {
    const response = await this.api.get(`/jobs/jobs/${jobId}/`);
    return response.data;
  }

  // Saved jobs live under jobs.completion_urls, mounted at
  // /api/v1/job-completion/ - not under /api/jobs/.
  async getSavedJobs(includeClosed: boolean = false) {
    const response = await this.api.get('/v1/job-completion/saved/', {
  ...
  async saveJob(jobId: number) {
    const response = await this.api.post(`/v1/job-completion/${jobId}/save/`);
    return response.data;
  }

  async unsaveJob(jobId: number) {
    const response = await this.api.delete(`/v1/job-completion/${jobId}/unsave/`);
    return response.data;
  }

  async isJobSaved(jobId: number) {
    const response = await this.api.get(`/v1/job-completion/${jobId}/is-saved/`);
    return response.data;
  }

  async registerPushToken(token: string, platform: string) {
    // NOTE: this must hit .../push-token/register/ (which actually persists
    // to the PushToken model) - the old .../register-token/ path this used
    // to call doesn't exist anywhere in the backend and always 404'd.
    // device_type must be one of the PushToken model's actual choices
    // ('ios' | 'android' | 'web'), which is exactly what Platform.OS already
    // returns - no translation to APNs/FCM provider names needed here.
    const response = await this.api.post('/notifications/push-token/register/', {
      token,
  ...
  async getNotifications(unreadOnly: boolean = false) {
    // Backend (worker_connect/notification_views.py::notification_list) reads
    // an 'is_read' query param, not 'unread_only' - that key was always
    // ignored server-side. Screens still worked because they also filter
    // client-side, but the server-side filter (and pagination cutoff) never
    // actually applied.
  ...
  async getMyCV() {
    const response = await this.api.get('/workers/cv/');
    return response.data;
  }

  async getMyCVDownloadUrl() {
    // Opened in an external browser tab, so the token travels as a query
    // param (see QueryParamTokenAuthentication on the backend) since the
    // axios auth header isn't attached outside this app's own requests.
    const token = await SecureStorage.getToken();
    if (!token) {
      throw new Error('Not authenticated');
    }
    return `${API_BASE_URL}/workers/cv/download/?token=${token}`;
  }
  ...
  // Browse available workers for a category, so the client can pick one
  // themselves (admin still confirms before it becomes a real assignment).
  async getAvailableWorkersForRequest(categoryId: number, latitude?: number | null, longitude?: number | null) {
    const params: Record<string, any> = { category: categoryId };
    if (latitude != null && longitude != null) {
      params.latitude = latitude;
      params.longitude = longitude;
    }
    const response = await this.api.get('/v1/client/browse-workers/', { params });
    return response.data;
  }
  ...
  async calculatePrice(data: {
    ...
    // Backend (clients/pricing_api.py::calculate_price) reads 'start_date'/
    // 'end_date', not 'service_start_date'/'service_end_date'.
    start_date?: string;
    end_date?: string;
  }) {
  ...
  // These three are registered under jobs.api_urls (mounted at /api/jobs/),
  // not clients.api_urls - '/clients/applications/...' never resolved.
  async getJobApplications(jobId: number) {
    const response = await this.api.get(`/jobs/client/jobs/${jobId}/applications/`);
    return response.data;
  }

  async acceptJobApplication(applicationId: number) {
    const response = await this.api.post(`/jobs/client/applications/${applicationId}/accept/`);
    return response.data;
  }

  async rejectJobApplication(applicationId: number) {
    const response = await this.api.post(`/jobs/client/applications/${applicationId}/reject/`);
    return response.data;
  }
  ...
  // Get conversation with specific user (same real endpoint as getMessages)
  ...
  async createSupportTicket(data: {
    ...
    const response = await this.api.post('/support/ticket/', data);
    return response.data;
  }
  ...
  // The backend requires confirm_email to equal the account's real email
  // as a confirmation check - without it this always 400s.
  async deleteAccount() {
